...
Persistent memory + ask-first skill authoring. Three-layer memory model: **raw/** (event stream), **knowledge/** (compiled facts as markdown), **graph/** (entity-relationship triples). FTS5 + vector + KG retrieval. Memory is passive (no background curator); skills are written only after you approve a proposal. Daily-audit loop, subject classification, schema linting.
