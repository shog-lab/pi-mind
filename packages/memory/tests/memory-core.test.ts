...
describe('MemoryCore', () => {
  ...
  describe('legacy migration', () => {
    it('migrates memory/ files to wiki/', async () => {
      ...
      const mc2 = new MemoryCore({
        groupDir: tmpDir,
        dbPath: path.join(tmpDir, '.pi-mind-index2.db'),
        freshDb: true,
        legacyMemoryDir: legacyDir,
      });

      // After flattening, non-compaction types go to wiki/ root
      expect(fs.existsSync(path.join(tmpDir, 'knowledge', 'old-fact.md'))).toBe(
        true,
      );
      expect(fs.existsSync(path.join(tmpDir, 'knowledge', 'old-note.md'))).toBe(
        true,
      );
