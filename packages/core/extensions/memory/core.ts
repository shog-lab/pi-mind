  ...
  async syncIndex(): Promise<void> {
    return withGroupLock(this.groupDir, async () => {
      ...
      for (const filePath of files) {
        ...
        // Skip the auto-generated knowledge/index.md. It's a system
        // file (regenerated by generateIndex() at the end of every
        // syncIndex), not a knowledge entry. Without this skip the
        // next syncIndex would see it lacking frontmatter, run
        // auto-heal (adding tier=L2, tags=[auto-healed]), then
        // INSERT it into memory_fts — and a subsequent retrieval
        // query would surface the auto-healed index page as a
        // (low-relevance) "long-term-memory" hit. Same policy as
        // rebuildKGFromFiles: index.md is not part of the curated
        // knowledge set. basename is exact so `fooindex.md` is not
        // affected. We also DELETE any pre-existing FTS5 / memory_meta
        // / memory_vectors row for index.md: an older syncIndex may
        // have already auto-healed + inserted it. The cleanup loop
        // below only removes rows for files that are no longer on
        // disk, so a defensive per-file delete is the only way to
        // evict a row whose file is still present.
