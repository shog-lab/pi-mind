...
export class MemoryCore {
  ...
  async syncIndex(): Promise<void> {
    return withGroupLock(this.groupDir, async () => {
      ...
      // The single auto-generated file in this repo: knowledge/index.md,
      // regenerated by generateIndex() at the end of every syncIndex.
      // We exclude it from FTS5 / vector / memory_meta so it never
      // surfaces as a retrieval hit AND so the next syncIndex doesn't
      // see it lacking frontmatter and run auto-heal (which would
      // promote it to a fake tier=L2, tags=[auto-healed] knowledge
      // entry). The skip is path-precise (not basename) so that:
      //   - user-written `knowledge/fooindex.md` is still indexed
      //   - user-written `knowledge/sub/index.md` is still indexed
      //   - `raw/**/index.md` is still indexed (syncIndex scans
      //     multiple dirs; raw/ has no auto-generated index file)
      const autoIndexPath = join(this.knowledgeDir, "index.md");
      for (const filePath of files) {
        currentFiles.add(filePath);
        if (filePath === autoIndexPath) {
          // Defensive: an older syncIndex may have already auto-healed
          // and INSERTed this path. The cleanup loop below only
          // removes rows for files that are no longer on disk, so a
          // per-file delete is the only way to evict a row whose
