...
export function runInit(opts) {
    ...
    linkInto(join(pkgRoot, "dist", "extensions"), join(hostRoot, ".pi", "extensions"), hostRoot, result, log, warn);
    linkInto(join(pkgRoot, "skills"), join(hostRoot, ".pi", "skills"), hostRoot, result, log, warn);
 ...
 */
function linkInto(srcDir, destDir, hostRoot, result, log, warn) {
 ...
 */
export function findHostRoot(pkgRoot, initCwd) {
    if (initCwd)
        return initCwd;
    let dir = pkgRoot;
    while (dir !== "/" && dir !== ".") {
